...
void ViewMeun(List L, List1 L1)//查询客户信息菜单
{
	...
	while (1)
	{
		...
		case 1:  system("cls");  char num[30]; printf("\n请输入所要查询的账号:  ");
			fgets(num,sizeof(num),stdin); ViewNum(num, L);      system("cls");  ViewMeun(L, L1);   break;   //按账号查询
		case 2:  system("cls");  char ID[20]; printf("\n请输入需要查询的身份证： ");
			fgets(ID,sizeof(ID),stdin); ViewID(ID, L);         system("cls");  ViewMeun(L, L1);   break;   //按身份证号查询
		case 3:  system("cls");  char name[10]; printf("\n请输入需要查询的姓名： ");
			fgets(name,sizeof(name),stdin); ViewName(name, L);  system("cls");  ViewMeun(L, L1);    break;  //按姓名查询
		case 4:  system("cls");  char tel[20]; printf("\n请输入需要查询的电话： ");
			fgets(tel,sizeof(tel),stdin); ViewTel(tel, L);      system("cls");  ViewMeun(L, L1);   break;   //按电话查询
